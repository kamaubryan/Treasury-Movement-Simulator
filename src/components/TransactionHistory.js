...
function TransactionHistory({ filterAccount, filterCurrency }) {
  ...
  const filteredTransactions = transactions.filter((tx) => {
    ...
    const matchesCurrency = filterCurrency
      ? tx.originalCurrency === filterCurrency ||
        tx.transferredCurrency === filterCurrency
      : true;
          ...
          <tbody>
            {filteredTransactions.map((tx) => (
              <tr key={tx.id}>
                ...
                <td>
                  {tx.originalCurrency}{" "}
                  {tx.originalAmount.toLocaleString("en-US", {
                ...
                <td>
                  {tx.originalCurrency !== tx.transferredCurrency ? (
                    <>
                      {tx.transferredCurrency}{" "}
                      {tx.transferredAmount.toLocaleString("en-US", {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}
                      <br />
